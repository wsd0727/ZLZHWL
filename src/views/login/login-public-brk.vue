<template>
  ...
  <!-- <div class="login"> -->
    ...
    >
      <div class="title">{{ systemName || "通钢智能仓储系统" }}</div>
      ...
      <div class="login-header">
        ...
        <!-- <div class="right" @click="forgotPassword">忘记密码</div> -->
      </div>
          ...
          >
            <template #append>
              <el-button
                @click="getTelCode"
                type="primary"
                :disabled="telCode != '获取验证码' ? true : false"
                link
              >
                {{ telCode }}
              </el-button>
      ...
      >
        <el-button
          :loading="loading"
          size="large"
          :type="loginForm.password ? 'primary' : 'info'"
          round
      ...
      <div class="term-wrap" v-if="false">
        <!-- <el-icon><CircleCheck /></el-icon> -->
        <el-icon color="#0c64eb"><SuccessFilled /></el-icon>
        同意
    ...
    >
      ...
      <Form
        ...
        v-model:formData="resetPasswords"
      ...
      <div class="passMes">
        请输入包含大写字母、小写字母、数字和特殊字符的8-16位的密码
      </div>
    ...
    <vxe-modal
      destroy-on-close
      v-model="modalForgotPassword.open"
      :width="modalForgotPassword.width"
      :height="modalForgotPassword.height"
      id="formModal"
      resize
      storage
      transfer
    ...
    >
      ...
      >
        <el-form-item prop="MOBILE" label="手机号">
          <el-input
            v-model.number="forgotPasswordForm.MOBILE"
            type="number"
            autocomplete="off"
            placeholder="请输入手机号"
            clearable
        ...
        <el-form-item prop="SUREPASSWORD" label="确认密码">
          <el-input
            v-model="forgotPasswordForm.SUREPASSWORD"
            placeholder="请确认密码"
            type="password"
            autocomplete="new-password"
            show-password
    ...
    <vxe-modal
      destroy-on-close
      v-model="showClauseModel"
      width="60%"
      height="60%"
      resize
      id="showClauseContentId"
      storage
      transfer
      show-zoom
      show-footer
...
// console.log(systemConfig.value?.systemCode)
...
const commonBg = computed(() => systemConfig.value?.systemCode == 'ZLZHWL'?proxy.getAssetsFile("login/zlbg.jpg"):proxy.getAssetsFile("login/bg.png"));
...
const zlappList = ref([
  // { icon: proxy.getAssetsFile("login/ios.png"), name: "苹果", type: "1" },
  { icon: proxy.getAssetsFile("login/az.png"), name: "安卓", type: "1" ,imgs:proxy.getAssetsFile("login/lasiji.png")},
...
console.log("包含子表", router.currentRoute.value.query);
...
const forgotPasswordRules = reactive({
  ...
  NEWPASSWORD: [
    { required: true, trigger: "blur", message: "请输入密码" },
    {
      ...
      message: "请输入包含大写字母、小写字母、数字和符号的8-16位数的密码",
      trigger: "blur",
    },
  ],
  SUREPASSWORD: [
