  ...
  <div class="app-wrap">
    <formGroup ref="formGroupRef" class="group" title="订单信息" v-model:visable="formOpen">
      <div class="group-content">
        ...
        <miniTable max-height="130" :data="goodsList" :mergeRow="['MATERIALNAME','SENDCOMPANY','RECVCOMPANY']">
          ...
          <template v-if="orderData.ASSIGNMENTUNIT != '4'">
            <vxe-column field="DTLNUM" title="订单量" align="right" width="120">
              <template #default="{row}">
                {{row.DTLNUM}}{{row.TRANSUNIT}}
              </template>
            </vxe-column>
            <vxe-column field="ENTRUSTNUM" title="委托剩余量" align="right" width="120" v-if="orderData.ASSIGNMENTMODE == '2'">
              <template #default="{row}">
                {{row.ENTRUSTNUM}}{{row.TRANSUNIT}}
              </template>
            </vxe-column>
            <vxe-column field="TRANSURPLUS" title="拉运剩余量" align="right" width="120" v-if="orderData.ASSIGNMENTMODE == '1'">
              <template #default="{row}">
                ...
                <li class="list-item" v-for="(item, index) in PZList" :key="index">
                  ...
                  <div class="content">
                    ...
                    <miniTable ref="PZTableRef" :min-height="40" :data="item.MATERIALPARAMLIST" :rules="PZTableRules">
                      ...
                      <vxe-column field="ENTRUSTNUM" title="委托量" align="center" min-width="25%" v-if="orderData.ASSIGNMENTMODE != '1' && orderData.ASSIGNMENTUNIT != '4'">
                        <!-- <template #default="{row}">
...
const props = defineProps({
  currentData: [Array, Object, String]
})
...
const { brandModel, orderTransportType } = proxy.useDict("brandModel", "orderTransportType")
const formGroupRef = ref(null)
const tableRef = ref(null)
const listHeight = ref(0)
const miniTableHeight = computed(() => listHeight.value - 130)
...
watch(formOpen, () => {
  nextTick(() => {
    listHeight.value = window.innerHeight - 168 - formGroupRef.value?.$el.clientHeight
  })
})
...
const getOrderData = () => {
  ...
  getOrderDetail({
    ...
    nextTick(() => {
      listHeight.value = window.innerHeight - 230 - formGroupRef.value.$el.clientHeight
    })
  })
}
getOrderData()
...
const getData = (marge = false) => {
  getCarrierGroup({
    ...
    Total.value = res.RESULT.TOTAL * 1
    nextTick(() => {
...
const AddPZ = ({ checked, isFull, row, selectRow }) => {
  if (checked) {
    let goods = orderData.value.ASSIGNMENTUNIT == '4' || orderData.value.ASSIGNMENTMODE == '1' ? goodsList.value.map(al => {
      return {
        BILLNO: al.BILLNO,
