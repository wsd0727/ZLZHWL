...
<template>
  ...
  <div class="app-wrap">
    <formGroup ref="formGroupRef" class="group" title="订单信息" v-model:visable="formOpen">
      <div class="group-content">
        <el-row class="content">
          ...
          <el-col :span="6" class="content-item">
            <div class="label">客商企业</div>
            <TextOverflow :content="orderData.CUSTOMERNAME" class="value" />
          </el-col>
          <el-col :span="6" class="content-item">
          ...
          <el-col :span="6" class="content-item">
            <div class="label">拉运方式</div>
            <div class="value">{{ selectDictLabel(orderTransportType, orderData.ASSIGNMENTMODE) }}</div>
          </el-col>
        ...
        <!-- 货物信息块  -->
        <miniTable max-height="130" :data="goodsList">
          ...
          <vxe-column field="NORIGTAXPRICE" title="单价" align="right" width="120">
            <template #default="{row}">
              ...
              {{row.NORIGTAXPRICE}}元/{{row.UNITPRICEUNIT}}
            </template>
          </vxe-column>
          ...
          <vxe-column field="ENTRUSTNUM" title="委托剩余量" align="right" width="120" v-if="orderData.ASSIGNMENTMODE == '2' && orderData.ASSIGNMENTUNIT != '4'">
            <template #default="{row}">
              {{row.ENTRUSTNUM}}{{row.TRANSUNIT}}
            </template>
          </vxe-column>
          <vxe-column field="TRANSURPLUS" title="拉运剩余量" align="right" width="120" v-if="orderData.ASSIGNMENTMODE == '1' && orderData.ASSIGNMENTUNIT != '4'">
            <template #default="{row}">
              {{row.TRANSURPLUS}}{{row.TRANSUNIT}}
            </template>
          </vxe-column>
          <vxe-column field="name" title="委托进度" align="left" width="420">
            <template #default={row}>
              <el-progress :percentage="((orderData.ASSIGNMENTMODE == '1' ? row.TRANSURPLUS : row.ENTRUSTNUM) / row.DTLNUM)*100" :format="()=>orderData.ASSIGNMENTUNIT != '4' ? `订单量 ${row.DTLNUM}${row.TRANSUNIT}` : ''" />
            </template>
          </vxe-column>
        </miniTable>
      </div>
    ...
    <div class="group-form" :style="{minHeight: `${listHeight}px`}">
            ...
            <div class="table">
              <template v-if="selectType == 'car'">
                <miniTable ref="tableRef" row-id="PK_CAR" :data="dataList" :height="miniTableHeight" show-overflow="tooltip" @change="AddPZ">
                  ...
                  <vxe-column title="状态" align="center" width="100" sortable>
                    <template #default="{row}">
                      <div :class="row.CARSTATUS !=0?'redColor':''">{{row.CARSTATUS == '0'?'空闲':row.CARSTATUS == '1'?'占用':''}}</div>
                    </template>
                  </vxe-column>
              ...
              <template v-else>
                <miniTable ref="tableRef" row-id="PK_DRIVER" :data="dataList" :height="miniTableHeight" show-overflow="tooltip" @change="AddPZ">
                  ...
                  <vxe-column field="STATUS" title="状态" align="center" width="100">
                    <template #default="{row}">{{row.STATUS == '0'?'空闲':'占用'}}</template>
                  </vxe-column>
            ...
            <el-scrollbar :height="PZHeight" v-if="PZList.length > 0">
              <ul class="list">
                <li class="list-item" v-for="(item, index) in PZList" :key="index">
                  ...
                  <div class="content">
                    <Form ref="PZRef" v-model:formData="PZList[index]" label-width="76px" :formConfig="systemConfig?.systemCode == 'ZLZHWL'?ZLPZConfig:PZConfig" :rules="PZFormRules">
                      <template #PK_CAR>
                        <el-select v-model="PZList[index].PK_CAR" filterable remote :remote-method="SearchBase" :loading="loading" @change="val=>setSearch(val,index)" @focus="SearchBase('')">
                      ...
                      <vxe-column field="ENTRUSTNUM" title="委托量" align="center" width="150" v-if="orderData.ASSIGNMENTUNIT != '4'">
                        <template #default="{row,rowIndex}">
                          <el-input v-model="row.ENTRUSTNUM" type="number">
                            <template #append>{{goodsList[rowIndex].TRANSUNIT}}</template>
                          </el-input>
                        </template>
                      </vxe-column>
      ...
      <vxe-modal destroy-on-close v-model="modelShow" width="60%" height="60%" id="FormModel" resize storage transfer show-zoom show-footer>
        ...
        <template #default>
          <miniTable ref="modelTableRef" height="410px" :data="orderHistory">
            ...
            <vxe-column field="TRANSPORTSTATUS" title="状态" width="100" align="center" show-overflow="tooltip">
              <template #default="{row}">
                {{ selectDictLabel(TRANSPORTSTATUS,row.TRANSPORTSTATUS) }}
              </template>
            </vxe-column>
            <vxe-column field="DRIVERNAME" title="司机" width="130" align="left" show-overflow="tooltip" />
            <vxe-column field="" title="线路" width="160" align="left" show-overflow="tooltip">
              <template #default="{row}">
                {{row.SUBLIST[0].LINENAME}}
              </template>
            </vxe-column>
...
import { QueryConfig, getPZCars, getPZDrivers, getBaseCar, getDriverGroup, getOrderHistory } from "#/WZC/Carrier/Stowage"
import { getOrderDetail, PZConfig, savePZDriver ,ZLPZConfig} from "#/WZC/OwnerCargo/Stowage"
...
const miniTableHeight = computed(() => listHeight.value - 120)
const PZHeight = computed(() => listHeight.value - 50)
const pageLoading = ref(false)
const formOpen = ref(true)
watch(formOpen, () => {
  nextTick(() => {
    listHeight.value = window.innerHeight - 240 - formGroupRef.value.$el.clientHeight
  })
})
...
const getOrderData = () => {
  ...
  getOrderDetail({
    ...
    nextTick(() => {
      listHeight.value = window.innerHeight - 280 - formGroupRef.value.$el.clientHeight
    })
  })
}
getOrderData()
    ...
    if (isFull) {
      PZList.value = selectRow.map(el => {
        return {
          ...
          CONTAINERSERVICE: orderData.value.CONTAINERSERVICE,
          PK_CAR: el.PK_CAR,
          BILLNO: BILLNO.value,
          PK_DRIVER: el.PK_DRIVER,
          ...
          ISONLINE: "0",
          PK_QUEUE: "",
          PK_COMPANY: orderData.value.PK_COMPANY,
          PROVIDER: "",
          BUSSTYPE: orderData.value.BUSSTYPE,
          EXPECTENTRYTIME: orderData.value.EXPECTENTRYTIME,
        EXPECTENTRYCOMPANY: orderData.value.EXPECTENTRYCOMPANY,
...
const validatePZList = () => {
  return new Promise(resolve => {
    if (!PZRef.value || !PZRef.value.length) {
      return proxy.$modal.msgError("请选择车辆或者司机");
    }
    let PZRefArr = []
...
const { TRANSPORTSTATUS, orderTransportType } = proxy.useDict("TRANSPORTSTATUS", "orderTransportType")
const total = ref(0)
const modelIndex = ref()
const handleHistory = index => {
  modelIndex.value = index
  let row = dataList.value[modelIndex.value]
  modeltitle.value = selectType.value == 'car' ? row.CARNO : row.DRIVERNAME
  getOrderHistory({
    ...
    DR: "0",
    PK_DRIVER: selectType.value == 'car' ? '' : row.PK_DRIVER,
    CARNO: selectType.value == 'car' ? row.CARNO : ''
  }).then(res => {
