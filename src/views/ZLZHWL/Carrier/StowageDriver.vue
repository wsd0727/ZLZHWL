        ...
        <miniTable max-height="130" :data="goodsList">
          ...
          <vxe-column field="RIGTAXPRICE" title="单价" align="right" width="120">
            <template #default="{row}">
              ...
              <div>{{row.RIGTAXPRICE}}元/{{row.UNITPRICEUNIT}}</div>
            </template>
          </vxe-column>
          ...
          <vxe-column field="name" title="委托进度" align="left" width="420">
            <template #default={row}>
              <el-progress :percentage="((row.ENTRUSTNUM - row.TRANSURPLUS) / row.ENTRUSTNUM)*100" :format="()=>`委托量 ${row.ENTRUSTNUM}${row.NSUMUNIT}`" />
            </template>
          </vxe-column>
        </miniTable>
        <!-- </template>
        <template v-else>
          <ul class="goodsList">
            <li class="goodsList-item" v-for="(item, index) in goodsList" :key="index">
              ...
              <div class="surplus" v-if="orderData.ASSIGNMENTUNIT != '4'">拉运余量 {{item.TRANSURPLUS}}{{item.NSUMUNIT}}</div>
            </li>
          </ul>
        </template> -->
      </div>
            ...
            <div class="table">
              <template v-if="selectType == 'car'">
                <miniTable ref="tableRef" row-id="PK_CAR" :data="dataList" :height="miniTableHeight" show-overflow="tooltip" @change="AddPZ">
                  ...
                  <vxe-column field="LOADWEIGHT" title="核载量(吨)" align="center" min-width="80"></vxe-column>
              ...
              <template v-else>
                <miniTable ref="tableRef" row-id="PK_DRIVER" :data="dataList" :height="miniTableHeight" show-overflow="tooltip" @change="AddPZ">
                  ...
                  <vxe-column field="STATUS" title="状态" align="center" width="100" sortable>
                    <template #default="{row}">
                      <div :class="row.STATUS !=0?'redColor':''">{{row.STATUS == '0'?'空闲':'占用'}}</div>
                    </template>
                  </vxe-column>
                  <vxe-column field="TSNUM" title="待运输单数" align="center" width="120" sortable>
                    <template #default="{row}">
                      <div :class="row.TSNUM ==0?'redColor':''">{{row.TSNUM}}</div>
                    </template>
                  </vxe-column>
                  <vxe-column field="name" title="拉运记录" align="center" width="90">
                    <template #default="{ rowIndex }">
            ...
            <el-scrollbar :height="PZHeight" v-if="PZList.length > 0">
              <ul class="list">
                <li class="list-item" v-for="(item, index) in PZList" :key="index">
                  ...
                  <div class="content">
                    <Form ref="PZRef" v-model:formData="PZList[index]" label-width="75px" :formConfig="PZConfig" :rules="PZFormRules">
                      <template #PK_CAR>
                      ...
                      <template #ISXX>
                        <el-switch v-model="PZList[index].ISXX" inline-prompt active-text="是" inactive-text="否" active-value="1" inactive-value="0" />
                      </template>
...
watch(formOpen, () => {
  nextTick(() => {
    listHeight.value =
      window.innerHeight - 170 - formGroupRef.value.$el.clientHeight;
    miniTableHeight.value =
      window.innerHeight - 350 - formGroupRef.value.$el.clientHeight;
...
const mapQueryConfig = computed(() =>
  QueryConfig.filter((el) => el.type == selectType.value || !el.type)
);
...
const dataList = ref([]);
const Total = ref(0);
const getListFn = computed(() =>
  selectType.value == "driver" ? getPZDrivers : getPZCars
);
...
const SearchFn = computed(() =>
  selectType.value == "car" ? getDriverGroup : getBaseCar
);
const SearchBase = (keyword) => {
  ...
  SearchFn.value({
    ...
    .then((res) => {
      selectType.value == "car"
        ? (DriverList.value = res.RESULT)
...
const AddPZ = ({ checked, isFull, row, selectRow }) => {
  if (checked) {
    ...
    if (isFull) {
      PZList.value = selectRow.map((el) => {
        return {
          ...
          systemCode: systemConfig.value?.systemCode,
        };
      });
    } else {
      let form = {
    ...
    if (isFull) {
      ...
      let index = PZList.value.findIndex((el) =>
        selectType.value == "car"
          ? el.PK_CAR == row.PK_CAR
          : el.PK_DRIVER == row.PK_DRIVER
      );
...
const PZDel = (index) => {
  ...
  proxy.$modal.confirm("是否删除当前配载数据？").then(() => {
    let row = dataList.value.find((el) =>
      selectType.value == "car"
        ? el.PK_CAR == delRow.PK_CAR
        : el.PK_DRIVER == delRow.PK_DRIVER
    );
...
const { TRANSPORTSTATUS } = proxy.useDict("TRANSPORTSTATUS");
const total = ref(0);
