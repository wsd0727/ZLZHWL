<!-- 线性一览表 -->
<template>
  <div class="app-wrap" v-loading="pagelLoading">
    <div class="app-table-box" v-if="!visibleFormPage">
      ...
      <div class="query-wrap" v-if="queryConfig.length">
        ...
        <div class="flex-row-end">
          ...
          <el-button @click="resetQuery" size="Default" v-if="systemConfig?.systemCode == 'ZLZHWL'">清空</el-button>
        </div>
      </div>
              ...
              <div class="block-list-item" v-for="(item,index) in dataList" :key="index">
                ...
                <el-row class="content">
                  ...
                  <el-col :span="24" class="content-item LinePlan">
                    <div class="line">
                      <el-progress class="percent" :stroke-width="7" text-inside :color="customColors" :percentage="custemProgress(row.BILLSTATUS)">
                        <div class="caricon">
    ...
    <div class="detailForm" v-else>
      <el-scrollbar class="">
        ...
        <div class="form-page-content">
          <detailCompontns :currentData="currentData" @close="closeFormPage" v-if="systemConfig.systemCode != 'ZLZHWL'" />
          <zlDetailCompontns :currentData="currentData" @close="closeFormPage" v-else />
        </div>
      </el-scrollbar>
    </div>
          ...
          <div class="LineInfo-item LineMap">
            <div class="map">
              ...
              <div class="input-card" v-if="mapData">
                ...
                <MapPlayBack ref="PlayBackRef" :AmapRef="AMap" :mapRef="MapRef" :path="mapData" />
              </div>
            </div>
          </div>
        </template>
...
import zlDetailCompontns from "./ZLDetails"
...
const getPageConfig = () => {
  PageConfigApi({
    ...
    baseQuery.value = getFormValue(QUERY)
...
const pagelLoading=ref(false)
const mapData = ref([])
const openMapMarker = index => {
  pagelLoading.value = true
  let data = dataList.value[index]
  getOrderMapLine({
    BILLNO: data.BILLNO,
    MODULEID: "MU230811295013",
...
const custemProgress = status => {
  if (status == 10) return 0
  if (status >= 95) return 100
  return 50
}
