          ...
          <ul class="goodsList">
            <li class="goodsList-item" v-for="(item, index) in goodsList" :key="index">
              <div class="head">
                ...
                <div class="price">{{item.NORIGTAXPRICE}}元/{{item.TRANSUNIT}}</div>
              </div>
              <div class="line">
                <el-progress :percentage="(item.ENTRUSTNUM / item.DTLNUM)*100" :format="()=>orderData.ASSIGNMENTUNIT != '4' ? `订单量 ${item.DTLNUM}${item.TRANSUNIT}` : ''" />
              </div>
              <div class="surplus" v-if="orderData.ASSIGNMENTUNIT != '4'">委托剩余量 {{item.ENTRUSTNUM}}{{item.TRANSUNIT}}</div>
            </li>
          </ul>
        </template> -->
      </div>
            ...
            <el-scrollbar :height="PZHeight" v-if="PZList.length > 0">
              <ul class="list">
                <li class="list-item" v-for="(item, index) in PZList" :key="index">
                  ...
                  <div class="content">
                    <Form ref="PZRef" v-model:formData="PZList[index]" label-width="76px" :formConfig="PZConfig" :rules="PZFormRules">
                      <template #PK_CAR>
...
import { getOrderDetail, PZConfig, savePZDriver } from "#/WZC/OwnerCargo/Stowage"
...
watch(formOpen, () => {
  nextTick(() => {
    ...
    miniTableHeight.value = (window.innerHeight - 350 - formGroupRef.value.$el.clientHeight) > 100 ? (window.innerHeight - 350 - formGroupRef.value.$el.clientHeight) : 200
