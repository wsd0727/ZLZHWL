    ...
    <el-row>
        ...
        <el-card class="box-card">
          ...
          <el-scrollbar :height="leftHight">
            <div class="leftText">
              ...
              <el-tree :props="{ label: 'LABEL', children: 'CHILDREN' }" node-key="VALUE" :data="options" @node-click="handleNodeClick" />
            </div>
          </el-scrollbar>
        </el-card>
      </el-col>
      <el-col :span="20">
        <el-card class="box-card">
          <!-- <template #header> -->
          <div class="card-header">
            <div v-if="Description != '' || textContent != ''">
              {{ vname }}
            </div>
          </div>
          <!-- </template> -->
...
import { TreeList, rightContent } from "#/system/problem";
...
function handleNodeClick(value) {
  vname.value = value.LABEL;
  sideIndex.value = value.VALUE;
...
function getRightList() {
  ...
  rightContent(data).then((res) => {
    if (res != null) {
      textContent.value = res.RESULT.ANSWER;
      Description.value = res.RESULT.CONTENT;
    } else {
      textContent.value = "";
      Description.value = "";
    }
