    ...
    <el-scrollbar :height="scrollHeight">
      <div ref="PrintRef" class="list">
        ...
        <miniTable ref="modelTableRef" :data="modelForm.SUBLIST" show-footer :footer-method="modelTableFooter">
          ...
          <vxe-column field="CSUP" title="车辆管理人" width="130" align="center" show-overflow="tooltip" />
        </miniTable>
      </div>
    </el-scrollbar>
  </div>
...
const handlePrint = () => {
  const topHtml = `<h2>运费付款 已对账凭证</h2>`
  tableRef.value.xTable.print({
    sheetName: '运费付款 已对账凭证',
...
const modalSubmit = () => {
  FormRef.value.submitForm().then(res => {
    if (!res) return
    let url = "/bms/meetFeeDtl/voucher"
