    ...
    <el-scrollbar :height="scrollHeight">
      <div ref="PrintRef" class="list">
        ...
        <miniTable ref="modelTableRef" :data="modelForm.SUBLIST" show-footer :footer-method="modelTableFooter">
          ...
          <vxe-column field="CDIGEST" title="摘要" min-width="200" align="left" show-overflow="tooltip" />
          <vxe-column field="CCODE" title="科目编码" min-width="150" align="center" show-overflow="tooltip" />
          <vxe-column field="MD" title="借方金额" min-width="150" align="right" show-overflow="tooltip">
            <template #default="{row}">
              {{ toThousands(row.MD) }}
            </template>
          </vxe-column>
          <vxe-column field="MC" title="贷方金额" min-width="150" align="right" show-overflow="tooltip">
            <template #default="{row}">
              {{ toThousands(row.MC) }}
            </template>
          </vxe-column>
          <vxe-column field="ND_S" title="数量" min-width="150" align="right" show-overflow="tooltip" />
          <vxe-column field="CSUP" title="承运商" min-width="150" align="left" show-overflow="tooltip" />
        </miniTable>
      </div>
    </el-scrollbar>
  </div>
...
const modelTableFooter = ({ columns, data }) => {
  return [
    columns.map((column, columnIndex) => {
      ...
      if (column.field == 'ND_S') return modelForm.value.TOTALDATA ? toThousands(modelForm.value.TOTALDATA.NDSSUM) : 0
      if (['MC', 'MD'].includes(column.field)) return modelForm.value.TOTALDATA ? toThousands(modelForm.value.TOTALDATA[column.field + "SUM"]) : 0
      return null
    })
  ]
}
...
const modalSubmit = () => {
  FormRef.value.submitForm().then(res => {
    if (!res) return
    let url = "/bms/meet-account-owner/voucher"
