<template>
  <div class="app-wrap">
    ...
    <vxe-modal destroy-on-close v-model="pageConfig.modalVisible" :width="pageConfig.modalW" :height="pageConfig.modalH" id="formModal" resize storage transfer show-zoom show-footer>
      ...
      <template #default>
        <MasterForm v-if="formConfig.formType == 'detail'" ref="MasterFormRef" v-model:formData="formConfig.formValue" :formConfig="formConfig.formColumns" :tableConfig="SubTableConfig" detail :infoConfig="[]" :showHistory="false" />
        <WorkflowTimeLine v-else-if="formConfig.formType == 'timeline'" :data="workDoData" />
        <template v-else>
          <MasterForm ref="MasterFormRef" v-model:formData="formConfig.formValue" :formConfig="formConfig.formColumns" :tableConfig="SubTableConfig" detail :infoConfig="[]" :showHistory="false" isWorkFlow @btnClick="btnClick" />
          <h3>审批意见</h3>
...
import { waitDoList, detailDo, enterDo, cancelDo, waitDoTableConfig, doStatusDict } from "#/JDWL/workflow/do";
...
const handleDetail = (row) => {
  ...
  getPageConfig({
    ...
    let { SLOTCFG, COLUMNS, BUTTON, SUBTABLE } = res.RESULT
