<template>
  <div class="bg-white app-wrap">
    <div class="nopnom">
      ...
      <div class="noPadding-right">
        ...
        <div class="group">
          ...
          <Vtable ref="tableRef" :tableCFG="tableConfig" :tableData="dataList" @dbClick="handleDetail" actionBar :actionBarWidth="220" @change="tableChange">
    ...
    <vxe-modal destroy-on-close v-model="pageConfig.modalVisible" :width="pageConfig.modalW" :height="pageConfig.modalH" id="formModal" resize storage transfer show-zoom show-footer>
      ...
      <template #default>
        <el-row :gutter="10">
          <Form ref="formRef" v-model:formData="form" labelWidth="60px" :formConfig="formConfig" :rules="rules" :detail="formType == 'detail'">
            <template #PK_USERGROUP>
              <el-tree-select v-model="form.PK_USERGROUPArr" :data="groupData" multiple collapse-tags :check-strictly="true" collapse-tags-tooltip filterable remote remote-show-suffix :remote-method="SearchGroup" @focus="SearchGroup('')" :render-after-expand="false" :props="{ label: 'LABEL', children: 'CHILDREN' }" value-key="VALUE" show-checkbox @change="setUSERGROUP" style="width:100%" />
            </template>
            <template #MENULIST>
...
import { UserList, UserDetail, addUser, editUser, delUser, statusUser, getAuthRole, updateAuthRole, deptTreeSelect, updateUserPwd, FormConfig, PWDConfig, TableConfig, getGROUP } from "#/JDWL/system/user";
...
const initForm = () => {
  ...
  Object.assign(formData, { PK_USERGROUPArr: [] })
...
const setUSERGROUP = () => {
  form.value.PK_USERGROUP = form.value.PK_USERGROUPArr.join(",")
}
...
const handleEdit = (row) => {
  ...
  UserDetail({ BILLNO }).then((res) => {
    ...
    if (formRef.value) formRef.value.resetForm();
    form.value = { ...baseForm.value, ...res.RESULT };
    form.value.PK_USERGROUPArr = form.value.PK_USERGROUP ? form.value.PK_USERGROUP.split(",") : []
