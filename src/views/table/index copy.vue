<template>
  <div class="app-wrap">
        ...
        <div class="bg-white position-r">
          <div class="tool-wrap">
            ...
            <div class="tool-wrap-right">
              ...
              <el-icon :size="14" style="margin-top: 2px" @click="colDrawer = true" class="mr10">
                <Setting />
              </el-icon>
            </div>
          </div>
          ...
          <slot name="default" :data="pageConfig.tableData" :config="pageConfig">
            ...
            <template v-else>
              <Vtable ref="VTableRef" v-if="!pageConfig.batchTable" :tableCFG="tableCFG" :tableData="pageConfig.tableData" :totalData="pageConfig.totalData" @change="tableChange">
                <template #expand>
            ...
            <div class="subtable-title" v-if="subTableList.subTableCol.length == 1">
              <div class="left"> </div>
          ...
          <vxe-pager size="mini" class-name="vxe-page-wrap" v-if="!subTableList.subTableCol.length" :page-size="pageInfo.pageSize" :page-sizes="[5, 10, 20, 30, 50, 100]" :current-page="pageInfo.currentPage" :total="pageInfo.totalResult" :layouts="['Total', 'PrevPage', 'JumpNumber', 'NextPage', 'Sizes']" @page-change="handlePageChange" />
        </div>
      </div>
    </div>

    <!-- 在当前页面中打开表单 -->
    <div class="form-page-wrap position-r" v-else>
      <el-scrollbar class="">
    ...
    <vxe-modal destroy-on-close v-model="reportCGF" :width="pageConfig.modalW" :height="pageConfig.modalH" id="reportModal" resize storage transfer show-zoom show-footer @close="closeModal">
      ...
      <template #default>
        <ReportFrom ref="RTableRef" :detail="formConfig.formType == 'DTL'" :reportFromData="reportFromData" :submitFrom="reportSubmit" />
      </template>
      <template #footer>
        <el-button size="default" @click="closeModal"> 关闭</el-button>
...
// console.log("包含子表", router.currentRoute.value);
...
function treeBtnEvent(t) {
  ...
  if (t == 3) {
    ...
    ElMessageBox.confirm(`您确定要该单据吗?`, "确认提示", {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    }).then(() => {
...
function saveColConfig() {
  ...
  let data = { ENGNAMESINFO: newData, NOTSHOWINFO: newData2 };
