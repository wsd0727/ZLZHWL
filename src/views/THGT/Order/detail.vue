        ...
        <div class="group" ref="groupRef0">
          ...
          <div class="group-content">
            <div class="StartEndInfo">
              ...
              <div class="SendTime">计划达到时间：{{form.SENDDATE}}</div>
            </div>
            <div class="LineInfo">
                ...
                <el-scrollbar height="500px">
                  <ul class="LineTime">
                    <div class="LineTime-item" v-for="(item, index) in timeLineList" :key="index">
                      ...
                      <div class="desc">{{item.LOADPOINTDESC}}</div>
                      <div class="imgList">
        ...
        <div class="group" ref="groupRef3">
          ...
          <div class="group-content" show-overflow="tooltip">
            <miniTable :data="warnList" min-height="180px">
              <vxe-column field="VNAME" title="预警类型" width="200" align="left">
              </vxe-column>
...
const addNewCarPoint = () => {
  const carMarker = new AMap.value.Marker({
    position: JSON.parse(newLocation.value),
    zIndex: 101,
...
const getMapData = () => {
  getOrderMapLine({
    ...
    let { LINEPOINTS, REALPOINTS } = res.RESULT
    if (LINEPOINTS) {
      mapPath.value = JSON.parse(LINEPOINTS)
      addPolyLine()
    } else if (REALPOINTS) {
      newLocation.value = `[${REALPOINTS}]`
      addNewCarPoint()
    }
  })
}
