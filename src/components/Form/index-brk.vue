<template>
  <div class="Form">
    <el-form :class="['Form', detail ? 'detail-form' : '']" ref="ruleFormRef" :model="formData" :inline="inline" :label-position="detail ? 'left' :labelPosition" :label-width="inline ? '70px' : labelWidth() " :rules="detail ? null : rules" :size="size" :disabled="detail" :validate-on-rule-change="false" @submit.prevent>
          ...
          <el-col :span="Ci.CONTROLS == 'ExTabs' || Ci.CONTROLS == 'subTable'? 24: Ci.COL * 1" v-if="isShowJudge(Ci)">
            ...
            <el-form-item v-else :prop="Ci.FIELD">
              ...
              <template v-if="!detail">
                ...
                <!-- ExNumber 数字输入框 -->
                <!-- <el-input-number v-else-if="Ci.CONTROLS == 'ExNumber'" :controls="false" v-model="formData[Ci.FIELD]" clearable type="number" :disabled="calcDISABLED(Ci)" :min="calcNumberMin(Ci)" :max="Number(Ci.MAXLENGTH)" @input="calcPoint($event, Ci)" @blur="numberBlur(Ci)" @clear="numberBlur(Ci)" style="width: 100%" @keydown.enter="enterNextEl">
                  <template #append v-if="Ci.SUFFIX && Ci.SUFFIX != ''">{{Ci.SUFFIX}}</template>
                </el-input-number> -->
                <div class="mo-input--number" v-else-if="Ci.CONTROLS == 'ExNumber'">
                  <el-input-number :controls="false" v-model="formData[Ci.FIELD]" clearable type="number" :disabled="calcDISABLED(Ci)" :min="calcNumberMin(Ci)" :max="Number(Ci.MAXLENGTH)" @input="calcPoint($event, Ci)" @blur="numberBlur(Ci)" @clear="numberBlur(Ci)" style="width: 100%" @keydown.enter="enterNextEl">
                  </el-input-number>

                  <div class="define-append" v-if="Ci.SUFFIX">{{Ci.SUFFIX}}</div>
                </div>
                ...
                <template v-else-if="Ci.CONTROLS == 'ExSelectModal'">
                  <!-- <el-dropdown ref="DropdownRef" size="small" trigger="click" placement="bottom-start" style="width: 100%" teleported>
                    <el-input v-model="formData[Ci.FIELD]" type="text" clearable @clear="SelectChange(Ci, null)" @keydown.stop="debounceGetSelectTableData(Ci)" :disabled="calcDISABLED(Ci)"  @keydown.enter="enterNextEl">
                      <template #append>
...
const emit = defineEmits(["update:formData", "select", "enter", "copyto", "upload"]);
const encryData = ref({});
...
const setDataArrs = () => {
  ...
  carConfig.value = formConfig.filter(el => el.CONTROLS === "ExCarNum");
...
const SelectChange = (config, val) => {
  ...
  if (CONTROLS != 'ExSelect' && CONTROLS != 'ExSelectMultiple') ParseOtherConfig(OTHER);
...
const calcPoint = (ev, config) => {
  let { FIELD, LINKAGE, MAXLENGTH, SLOTCFG } = config,
    point = 2;
  props.formData[FIELD] = Number(ev) // props.formData[FIELD] * 1
...
// 级联计算
const numberBlur = ({ FIELD, LINKAGE, MAXLENGTH }) => {
  if (!LINKAGE || /DAYCALC|DAYSTAGE/.test(LINKAGE)) return;
  // 小数点控制
  if (/POINT/.test(LINKAGE)) {
    try {
      const { POINT = 2 } = JSON.parse(LINKAGE);
      props.formData[FIELD] = !props.formData[FIELD] ? 0 : Number(props.formData[FIELD].toFixed(POINT));
    } catch (err) {
      console.error("Error:POINT解析", err);
    }
  }
