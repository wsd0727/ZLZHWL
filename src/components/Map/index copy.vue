<template>
  <div class="map">
    ...
    <div class="btn" v-if="!detail && (type == 'circle' || type == 'polygon')">
      <el-button @click="initEdit">初始化</el-button>
      <template v-if="EditRef!= null">
        <el-button @click="startEdit">开始编辑</el-button>
        <el-button @click="endEdit">结束编辑</el-button>
      </template>
    </div>
  </div>
</template>
...
<script setup name=''>
// 高德地图key   	ad52cd2ca0c59d1f83ae03abb84c0796
...
// 清除全部覆盖物
const cleatAllMarker = () => MapRef.value?.clearMap()
...
const initEdit = (center = undefined) => {
  if (props.type == '') return
  let type = props.type;
  if (center == undefined) center = MapRef.value.getCenter().toArray()
  cleatAllMarker()
  switch (type) {
    case "point": // 点
      addPoint(center)
...
// 主动暴露方法
defineExpose({ type: 'map', cleatAllMarker, addMapMarker });
</script>
