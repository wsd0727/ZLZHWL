...
watch(() => props.modelValue, val => {
  if (val) {
    ...
    fileList.value = list.map(item => {
      if (typeof item === "string") item = { name: item, url: resetImgUrl(item) };
      return item;
    });
  } else {
    fileList.value = [];
...
function handleUploadSuccess(res, file) {
  if (res.STATUS == '200') {
    uploadList.value.push({ name: res.RESULT.FILEPATH, url: (res.RESULT.FILEPATH.substr(0, 4) == 'http' || res.RESULT.FILEPATH.charAt(0) == '/') ? res.RESULT.FILEPATH : `/${res.RESULT.FILEPATH}` });
