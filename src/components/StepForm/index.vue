<template>
  <template v-if="open">
    ...
    <teleport to="#custemPageBtn" :disabled="!(active == 0 || pageList.length - 1 > active)">
      <el-button v-if="active == 0 || pageList.length - 1 > active" @click="Next" :disabled="config.CHOOSE2SUB?SelectData.length == 0:false" type="primary" size="default">下一步</el-button>
    </teleport>
  ...
  <div class="StepForm">
    <div class="step">
      <div class="step-item" v-for="(item, index) in pageList" :key="item.BILLNO">
        <keep-alive>
          <template v-if="active == index">
            <template v-if="item.VCODE == 'createdSuttle'">
              <component v-for="(multiItem,multiIndex) in margeFormData " :key="multiIndex" ref="ComRef" :is="MasterForm" :MODULE="item.PK_MODULE" :PAGE="item.BILLNO" hasConfig :Config="item" :formConfig="item.COLUMNS" v-model:formData="multiFormData[multiIndex]" :formRules="FormRules" :tableConfig="SubTableConfig" :tableRules="SubTableRules" :infoConfig="InfoConfig" :showHistory="false" @change="handleChange"></component>
            </template>
            <component v-else ref="ComRef" :is="item.GROUPNO == 'FM'?MasterForm:ModalTable" :MODULE="item.PK_MODULE" :PAGE="item.BILLNO" hasConfig :Config="item" :formConfig="item.COLUMNS" v-model:formData="FormData" :formRules="FormRules" :tableConfig="SubTableConfig" :tableRules="SubTableRules" :infoConfig="InfoConfig" :showHistory="false" @change="handleChange" @queryTotalTableData="queryTotalTableData"></component>
          </template>
        </keep-alive>
      </div>
    </div>
...
const InitConfig = () => {
  ...
  margeFormData.value.forEach((item, index) => {
    multiFormData.value[index] = deepClone(formBase)
    multiFormData.value[index] = Object.assign(multiFormData.value[index], margeFormData.value[index])
  })
...
const handleChange = res => {
  let newRes = Array.isArray(res) ? res : [res]
  SelectData.value = newRes
}

const totalTableData = ref([])
const queryTotalTableData = (val) => {
  totalTableData.value = val
}
...
const Next = () => {
  if (props.config.CHOOSE2SUB) {
    margeFormData.value = { [props.config.CHOOSE2SUB]: SelectData.value }
  } else {
    margeFormData.value = { _ALLTABLEDATA: totalTableData.value }
  }
  ...
  if (StepBtn.value.length) {
    ...
    if (CHOOSE2SUB) {
      ...
      data._ALLTABLEDATA = totalTableData.value
      margeFormData.value = { [props.config.CHOOSE2SUB]: totalTableData.value }
    }
    if (url) {
